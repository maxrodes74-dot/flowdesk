/**
 * MTR-289: REST API v1 — Document Export
 * POST /api/v1/documents/:id/export — trigger PDF/DOCX generation
 */
...
export async function POST(
  ...
  if (format === 'pdf') {
    ...
    const storagePath = `${freelancer_id}/${id}/v${doc.version}.pdf`;
    ...
    return apiSuccess({ file_url: urlData.publicUrl, format: 'pdf' });
  }

  return apiError('Format not yet supported', 501);
}
...
<body>
  ...
  ${bodyHTML}
  <div class="footer">
    Generated by ScopePad · ${new Date().toLocaleDateString()}
  </div>
</body>
</html>`;
}
...
function renderInvoice(
  ...
  const total = (metadata.total as number) || lineItems.reduce((sum, li) => sum + ((li.quantity as number) || 0) * ((li.rate as number) || 0), 0);
  return `
    <div class="section">
    ...
    <table>
      ...
      <tbody>
        ${lineItems.map((li) => `<tr><td>${li.description}</td><td style="text-align:right">${li.quantity}</td><td style="text-align:right">$${Number(li.rate).toFixed(2)}</td><td style="text-align:right">$${(((li.quantity as number) || 0) * ((li.rate as number) || 0)).toFixed(2)}</td></tr>`).join('')}
...
// Support for NextResponse import needed
