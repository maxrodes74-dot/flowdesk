...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If session exists, email confirmation is disabled - redirect to garden
    if (data.session) {
      router.push("/garden");
      router.refresh();
    }
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text)' }}>Create your terrarium</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
          Start growing your knowledge garden
        </p>
      </div>
      ...
      {emailSent && (
        <div className="bg-green-900/30 border border-green-700/50 text-green-300 text-sm rounded-lg p-3">
          ...
          <p>We&apos;ve sent a confirmation link to <span className="font-medium">{email}</span>. Please confirm your email to complete signup.</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2" style={{ background: 'var(--surface)', color: 'var(--text-secondary)' }}>
                Or sign up with
              </span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full border font-medium py-2.5 rounded-lg transition flex items-center justify-center gap-2 disabled:opacity-50"
